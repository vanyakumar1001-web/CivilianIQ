...
const EXAMPLES: Record<'IN' | 'US' | 'CN' | 'RU' | 'BR' | 'GB', string[]> = {
  ...
  RU: [
    ...
    'гаишники остановили машину',
    'меня задержали без причины',
    'полиция хочет обыскать квартиру',
  ],
  BR: [
    'policial me parou e quer meu celular',
    'policia rodoviaria me multou',
    'fui detido sem explicação',
    'policia quer revistar minha casa',
  ],
  GB: [
    'police stopped me and want to search my bag',
...
export default function HomePage() {
  ...
  async function handleSearch(e: FormEvent<HTMLFormElement>) {
    ...
    setGlossary([]);
    try {
      const res = await fetch(functionUrl('classify'), {
        method: 'POST',
  ...
  const noResultsKey =
    preference.country === 'US'
      ? 'noResults_US'
      : preference.country === 'CN'
        ? 'noResults_CN'
        : preference.country === 'RU'
          ? 'noResults_RU'
